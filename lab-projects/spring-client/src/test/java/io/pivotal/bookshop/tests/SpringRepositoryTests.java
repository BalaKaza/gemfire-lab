...
public class SpringRepositoryTests {
    ...
    public void joinQueryTest() {
        SelectResults<BookMaster> res = bookRepo.findLowStockBooks(2);
        List<BookMaster> results = new ArrayList<>();
        for (Struct book : res) {
            results.add(
                    new BookMaster(
                    (Integer) book.get("ItemNumber"),
                    book.get("description").toString(),
                    (Float) book.get("retailCost"),
                    (Integer) book.get("yearPublished"),
